...
""";
        ...
        CancellationToken ct)
    {
        ...
        sb.AppendLine($"  get_estimate_details(estimate_id) → full labor+equipment rows for a past estimate, including peakQty per row — call AFTER search_estimates to compare historical job to current");
        sb.AppendLine("REVIEW / LEFT ON THE TABLE: when asked to review vs past jobs / check missing items / 'what am I leaving on the table' / 'Review this estimate against similar past awarded jobs': 1. Call search_estimates(client=<current client>, city=<current city>, state=<current state>, status=Awarded) — find past awarded jobs at this location. 2. Pick the most relevant awarded job (highest grandTotal or most similar name). Call get_estimate_details on it. 3. Compare its laborRows.peakQty to CURRENT LABOR ROWS — flag positions missing entirely OR where peakQty is lower. 4. Compare its equipmentRows to CURRENT EQUIPMENT — if CURRENT EQUIPMENT is none, ALL past equipment is missing. 5. For each missing/reduced labor position: dollar impact = (past peakQty - current qty) × past stRate × 8hrs/day × past days. 6. For each missing equipment item: dollar impact = past rate × past qty × past days. 7. Sum → state 'Based on the [Job Name] job, you may be leaving approximately $X on the table.' 8. List every gap with its dollar value. Offer to add missing labor via add_labor_rows.");
        sb.AppendLine("REVIEW REVENUE IMPACT (fallback — use when no past job found): At the END of any review response, after listing missing positions, include a dollar estimate for each missing position (benchmark ST rate × 8 hrs/day × job Days from ESTIMATE header × typical headcount: 1 for GF/PM/NDT/Crane Op, 2 for journeymen, 2 for helpers, 2 for Safety Watch). Sum all missing positions and state: 'These missing positions represent approximately $X in potential revenue.'");
